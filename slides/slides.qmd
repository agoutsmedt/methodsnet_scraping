...
datatable(bis_pages,
          ...
          options = list(
            ...
            pageLength = 4, # This sets the initial number of rows displayed to 5
            dom = "tip"))
```
...
month <- "10"
year <- 2024
url_second_page <- glue("{bis_website_path}?fromDate={day}%2F{month}%2F{year}&cbspeeches_page={page}&cbspeeches_page_length=25")
```
...
### Scraping all the pages {.scrollable}

```{r all-pages, eval=TRUE}
starting_url <- glue("{bis_website_path}?fromDate={day}%2F{month}%2F{year}&cbspeeches_page=1&cbspeeches_page_length=25")
...
for(page in 1:nb_pages){
  url <- glue("{bis_website_path}?fromDate={day}%2F{month}%2F{year}&cbspeeches_page={page}&cbspeeches_page_length=25")
